...
if ($chinese) {
    $messageLabel.Text = "CMCL 需要 Java 运行时环境才能正常运行，是否自动下载安装 Java？"
} else {
    $messageLabel.Text = "Running CMCL requires a Java runtime environment. `nDo you want to download and install Java automatically?"
}
...
do {
    $tempFileName = "cmcl-java-$(Get-Random).zip"
