...
public abstract class ModrinthManager {
    ...
    protected abstract ModrinthSection getSection();
    ...
    public JSONObject search(String searchContent, int limit) {
        ...
        try {
            JSONObject jsonObject = new JSONObject(Utils.get(ROOT + "search?query=" + URLEncoder.encode(searchContent, StandardCharsets.UTF_8.name()) + "&limit=" + (limit < 0 ? 50 : limit) + "&facets=" + URLEncoder.encode("[[\"project_type:" + getSection().projectType + "\"]]", StandardCharsets.UTF_8.name())));
        ...
        for (int i = list.size() - 1; i >= 0; i--) {
            try {
                ...
                if (versions != null && versions.length() > 0) {
                    ...
                    list2.sort((o1, o2) -> -Utils.tryToCompareVersion(o1, o2));
                ...
                CFManager.printOne(i + 1, result.optString("title"), result.optString("description"), Ansi.ansi().fg(Ansi.Color.RED).a(result.optString("author")).toString(), gameVersion, null);
            } catch (Exception e) {
    ...
    public void printInformation(@Nullable JSONObject bySearch, JSONObject byID, @Nullable String modName, @Nullable String modId) {
        ...
        /*JSONArray modLoaders = bySearch.optJSONArray("modLoaders");
...
*/
    ...
    public JSONObject getByID(String id) throws IncorrectCategoryAddon, IOException {
        JSONObject mod;
        mod = new JSONObject(Utils.get(ROOT + "project/" + id));
    ...
    private String getNameAllLowerCase() {
